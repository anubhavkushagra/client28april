...
public:
    ...
    bool connect() {
        ...
        if (stubs_.empty()) {
            ...
            int retry_count = 0;
            const int expected_nodes = 8; // Architecturally, we require 100% capacity for 1M+ TPS
            ...
            while (true) {
                ...
                disc_req.set_client_version("v8-batch");
...
int main(int argc, char** argv) {
    if (argc < 4) {
        std::cout << "Usage: ./kv_client_batch <THREADS> <SECONDS> <SERVER_IP> [BATCH_SIZE]\n";
        return 0;
    }
